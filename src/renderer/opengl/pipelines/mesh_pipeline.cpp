...
namespace renderer {
    namespace Opengl {
        MeshDrawPipeline::MeshDrawPipeline() : Pipeline() {};

        void MeshDrawPipeline::draw(Mesh &mesh, uint flags = Mesh_Draw_All) {
            draw(program, mesh, flags);
        }

        void MeshDrawPipeline::draw(Program *drawProgram, Mesh &mesh, uint flags = Mesh_Draw_All) {
