...
namespace memory {
	...
	void* FreeListAllocator::Allocate(const std::size_t size, const std::size_t alignment) {
		...
		#ifdef DEBUG
		console::info("A", "\t@H ", (void*)headerAddress, "\tD@ ", (void*)dataAddress, "\tS ", ((FreeListAllocator::AllocationHeader *)headerAddress)->blockSize,  "\tAP ", alignmentPadding, "\tP ", padding, "\tM ", m_used, "\tR ", rest);
		#endif
	...
	void FreeListAllocator::Free(void* ptr) {
		...
		#ifdef DEBUG
		console::info("F", "\t@ptr ",  ptr,"\tH@ ", (void*) freeNode, "\tS ", freeNode->data.blockSize, "\tM ", m_used);
		#endif
	}
	...
	void FreeListAllocator::Coalescence(Node* previousNode, Node * freeNode) {
		if (freeNode->next != nullptr && (std::size_t) freeNode + freeNode->data.blockSize == (std::size_t) freeNode->next) {
			...
			#ifdef DEBUG
			console::info("\tMerging(n) ", (void*) freeNode, " & ", (void*) freeNode->next, "\tS ", freeNode->data.blockSize);
			#endif
		}
		...
		if (previousNode != nullptr && (std::size_t) previousNode + previousNode->data.blockSize == (std::size_t) freeNode) {
			...
			#ifdef DEBUG
			console::info("\tMerging(p) ", (void*) previousNode, " & ", (void*) freeNode, "\tS ", previousNode->data.blockSize);
			#endif
		}
	}
